...
public class BrickRow : Control, ITemplateEditor
{
    ...
    [
        ...
        new("incident_limit",  "Inc. Limit",  "Session incident limit (e.g. 17x — empty when there is no limit)"),
        new("tire",            "Tire",        "Tire compound currently fitted (e.g. Soft, Primary — empty if no choice available)"),
        new("car_number",      "Car #",       "Your car number in the session (e.g. #47)"),
        new("class",           "Class",       "Your car class short name (e.g. GT3, GTP, LMP2)"),
        new("license",         "License",     "Your iRacing license and safety rating (e.g. A 2.00)"),
        new("series",          "Series",      "The series name from iRacing (e.g. Porsche Carrera Cup Fixed)"),
        new("delta",           "Delta",       "Current lap vs your best lap (e.g. Δ +0.234 or Δ -0.456 — only shown on track with a valid best lap)"),
        new("stint_time",      "Stint",       "Time on track since last pit stop or joining the session (e.g. Stint 32:14)"),
        new("gap_ahead",       "Gap Ahead",   "Time gap to the car directly ahead in race position (e.g. +1.3s — empty when P1 or gap unknown)"),
        new("speed_kmh",   "km/h",       "Current speed in km/h"),
        new("speed_mph",   "mph",        "Current speed in mph"),
        new("gear",        "Gear",       "Current gear (R, N, 1, 2...)"),
        new("rpm",         "RPM",        "Engine RPM (e.g. 8450 rpm — empty at 0)"),
        ...
        new("fuel_laps",   "Fuel Laps",  "Estimated laps of fuel remaining based on current usage rate"),
        ...
        new("drivers",     "Drivers",    "Total number of competitors in session (e.g. 24)"),
        new("laps_down",   "Laps Down",  "Laps behind race leader (e.g. 2L down — empty when on the lead lap)"),
        new("gap_leader",  "Gap Leader", "Time behind race leader (e.g. +12.3s — empty when leading)"),
        new("session_time","Session Time","Elapsed session time (e.g. 12:34)"),
        new("lap_time",    "Lap Time",   "Current in-progress lap time (e.g. T 1:23.4 — empty off track)"),
        new("wind",        "Wind km/h",  "Wind speed in km/h and compass direction (e.g. 19 km/h NW — empty when calm)"),
        new("wind_mph",    "Wind mph",   "Wind speed in mph and compass direction (e.g. 12 mph NW — empty when calm)"),
        new("humidity",    "Humidity",   "Air humidity percentage (e.g. 65% — empty when 0)"),
        new("wet",         "Wet",        "Shows 'Wet' when iRacing has declared a wet track; empty otherwise"),
        new("time_of_day", "Time of Day","In-sim clock in 12-hour format (e.g. 2:34 PM)"),
        new("sr",              "SR",              "Your current Safety Rating (e.g. SR 3.44 — empty if unranked)"),
        new("sr_avg5",         "SR Avg 5",        "Average SR from your last 5 race-end values (e.g. SR5 3.21)"),
        new("sr_avg10",        "SR Avg 10",       "Average SR from your last 10 race-end values (e.g. SR10 3.15)"),
        new("sr_avg_custom",   "SR Avg (Custom)", "Average SR over the custom window set in App settings"),
    ];
